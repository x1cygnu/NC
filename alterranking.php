...
$T->Insert(2,3,"Tag");
$T->Insert(3,3,"Name");
$T->Insert(4,3,new Link("ranking.php?type=1&from={$GET['from']}&to=${GET['to']}&ord=0","Points"));
$T->Insert(5,3,"Cntd");
$T->Insert(6,3,new Link("ranking.php?type=1&from={$GET['from']}&to=${GET['to']}&ord=1","Members"));
$T->Insert(7,3,"URL");
...
foreach ($R as $P)
{
    ...
    if ($P['Countdown']<7)
        $T->Insert(5,$i,$P['Countdown']);
    $T->Insert(6,$i,$P['NoMembers']);
    if ($P['URL']!="")
    $T->Insert(7,$i,new Link($P['URL'],$P['URL']));
}
