...
{
    $T->Insert(1,2,new Link("logaddr.php?i=$u","" . ceil($u/100)));
    $v=$u+50;
    $T->Insert(1,2,new Link("logaddr.php?i=$v","."));
}
$T->Insert(1,3,"Name");
$T->Insert(2,3,"ID");
$T->Insert(3,3,"IP");
...
foreach ($Logs as $Log)
{
    ...
    $T->Insert(7,$row," ");
    $T->Insert(7,$row,new Link("multilogins.php?ipv=" . $IP,"View"));
    $T->Insert(7,$row," ");
    $T->Insert(7,$row,new Link("multiexceptions.php?aid={$Log['AID']}&ip=$IP","Add"));
}
