...
ForceActivePlayer($sql, $H, "oldmap.php");
ForceFrozen($sql, $H);
...
$M->Insert(1,1,new Link("oldmap.php?x=$newl&y=$newt&r=$bio",
			new Image('IMG/tlb.png',"TL")));
$M->Insert(1,3+2*$bio,new Link("oldmap.php?x=$newl&y=$newb&r=$bio",
			new Image('IMG/blb.png',"BL")));

$M->Insert(3+2*$bio,1,new Link("oldmap.php?x=$newr&y=$newt&r=$bio",
			new Image('IMG/trb.png',"TR")));
$M->Insert(3+2*$bio,3+2*$bio,new Link("oldmap.php?x=$newr&y=$newb&r=$bio",
...
for ($tx = 1; $tx <= $bio*2+1; $tx++)
{
    if ($tx==$bio+1)
    {
	$M->Insert(1+$tx,1,new Link("oldmap.php?x=$x&y=$newt&r=$bio",
				new Image('IMG/tb.png',"up")));
	$M->Insert(1+$tx,3+$bio*2,new Link("oldmap.php?x=$x&y=$newb&r=$bio",
				new Image('IMG/bb.png',"down")));
	$M->Insert(1,$tx+1,new Link("oldmap.php?x=$newl&y=$y&r=$bio",
				new Image('IMG/lb.png',"left")));
	$M->Insert(3+$bio*2,$tx+1,new Link("oldmap.php?x=$newr&y=$y&r=$bio",
...
$stars=starsystem_show_range($sql, $_SESSION['PID'], $x, $y, $bio);
