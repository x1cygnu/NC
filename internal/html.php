...
class Text
{
  ...
  function Text($sV="") {$this->sText=strval($sV);}
}
...
class EventContainer extends Container
{
  ...
  function onMouseMove($sEvent) {$this->aEvent["onmousemove"]=$sEvent;}
  ...
  function onFocus($sEvent) {$this->aEvent["onfocus"]=$sEvent;}
  ...
  function addonFocus($sEvent) {$this->aEvent["onfocus"].=';'.$sEvent;}
...
class HTML extends NamedContainer
{
  ...
  function Draw()
  {
    ...
    echo "</head><body"; NamedContainer::Draw(); echo ">\n";
    //	if ($this->bCentered)
    //	    echo "<center>\n";
    Container::Draw();
    //	if ($this->bCentered)
  ...
  function AddStyle($sFileName)
  {
    ...
    {
      $sStylePrefix=extractCookie($this->sStyleThemePrefix . '_StyleTheme');
      if ($sStylePrefix!="")
        $sFileName="themes/" . $sStylePrefix . "/" . $sFileName;
      else
        $sFileName="themes/default/" . $sFileName;
    }
    if ($bv=!in_array($sFileName,$this->aStyles))
...
class Table extends NamedContainer
{
  ...
  function __clone()
  {
    foreach ($this->aLines as $Y => $row)
      foreach ($row as $X => $cell)
      $this->aLines[$Y][$X]=clone $cell;
  }
  ...
  function Set(/*$iX, $iY, $oCell*/)
  {
    ...
    $oOldCell=$this->aLines[$iY][$iX];
    $oCell->iRowSpan=$oOldCell->iRowSpan;
    $oCell->iColSpan=$oOldCell->iColSpan;
  ...
  function Draw()
  {
    ...
    {
          if (isset($this->aRowPrefix[$iRowNum]))
...
/*		if (isset($this->aCellLink[$iRowNum][$iColNum]))
	    ...
	    echo "</tr>";
	    if (isset($this->aRowSuffix[$iRowNum]))
