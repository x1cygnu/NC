...
function fleet_compute_combat_arr($fleetAndPlayer) {
	$F=$fleetAndPlayer;
	foreach ($anyships as $S) {
		$AV += get_AV($F[$S]);
		$DV += get_DV($F[$S]);
	}
	$Amod = Attack($F['Attack'])*0.01*(1+0.015*$F['Physics']+0.01*$F['PL']);
	$Dmod = Defence($F['Defence'])*0.01*( pow(1.02,$F['Mathematics'])+0.01*$F['PL']);
	$AV *= $Amod;
	$DV *= $Dmod;
	$ans['AV']=$AV;
	$ans['DV']=$DV;
...
function fleet_chance($Me, $Opponent) {
	$swapped=false;
	if ($Me['AV']<$Opponent['AV']) {
		$swapped=true;
		$WAV=$Opponent['AV'];
		$LAV=$Me['AV'];
	} else {
	...
	if ($WAV>$LAV*2.0)
		$chance=1;
	elseif ($WAV==0) //and $LAV==0
		$chance=0.5;
	else
		$chance=(1+sin(0.5*3.141593*($WAV-$LAV)/$LAV))/2;
	if ($swapped)
		return 1-$chance;
	else
		return $chance;
...
function fleet_damage($Me, $Opponent, $aggresiveness, $winModifier) {
	if ($Me['DV']>0) {
		...
		$ratio = ($aggresiveness*$EDDV/$EADV)*$winModifier;
...
function fleet_battle(&$result, &$fleetA, &$fleetD, &$resultFleetAWin, &$resultFleetDWin, &$resultFleetALoose, &$resultFleetDLoose, $Aattmod, $Adefmod, $Aphysic, $Amath, $APL, $Dattmod, $Ddefmod, $Dphysic, $Dmath, $DPL, $mission=0, $PlanetAttack=100, $PlanetDefense=100)
{
	...
	$ADefmod=$Adefmod*0.01*(pow(1.02,($Amath-$Dmath))+0.01*$APL);
	$DDefmod=$Ddefmod*0.01*(pow(1.02,($Dmath-$Amath))+0.01*$DPL);
...
function fleet_create(&$sql, $owner, $Vpr, $Int, $Fr, $Bs, $Drn, $CS, $Tr) {
	$Vpr=makeinteger($Vpr);
	$Int=makeinteger($Int);
	$Fr=makeinteger($Fr);
	$Bs=makeinteger($Bs);
	...
	$sql->query("INSERT INTO NC_Fleet VALUES(NULL, $owner, $Vpr, $Int, $Fr, $Bs, $Drn, $CS, $Tr)");
	$FID = $sql->id();
	$sql->query("INSERT INTO NC_FleetGroup VALUES($FID,NULL)");
	$FGID = $sql->id();
}

function fleet_update(&$sql, $FID, $owner, $Vpr, $Int, $Fr, $Bs, $Drn, $CS, $Tr) {
	$Vpr=makeinteger($Vpr);
	$Int=makeinteger($Int);
	$Fr=makeinteger($Fr);
	$Bs=makeinteger($Bs);
	...
	$owner=makeinteger($owner);
	$FID = makeinteger($FID);
	$sql->query("UPDATE NC_Fleet SET Owner=$owner, Vpr=$Vpr, `Int`=$Int, Fr=$Fr, Bs=$Bs, Drn=$Drn, CS=$CS, Tr=$Tr WHERE FID=$FID");
}

function fleet_update_arr(&$sql, $FID, $arr) {
	$Vpr=makeinteger($arr['Vpr']);
	$Int=makeinteger($arr['Int']);
	$Fr=makeinteger($arr['Fr']);
	$Bs=makeinteger($arr['Bs']);
	$Drn=makeinteger($arr['Drn']);
	$CS=makeinteger($arr['CS']);
	$Tr=makeinteger($arr['Tr']);
	$owner=makeinteger($arr['Owner']);
	$FID = makeinteger($FID);
	$sql->query("UPDATE NC_Fleet SET Owner=$owner, Vpr=$Vpr, `Int`=$Int, Fr=$Fr, Bs=$Bs, Drn=$Drn, CS=$CS, Tr=$Tr WHERE FID=$FID");
}

function fleet_get(&$sql, $FID) {
	$FID=makeinteger($FID);
	$F = $sql->query("SELECT * FROM NC_Fleet " .
			" WHERE FID=$FID");
	return $F[0];
}

function fleet_get_all(&$sql, $FID) {
	$FID=makeinteger($FID);
	$F = $sql->query("SELECT * FROM NC_Fleet F " .
			" JOIN NC_FleetGroup FG ON F.FID=FG.FID" .
...
function fleet_get_group(&$sql, $FID) {
	$FID=makeinteger($FID);
	$F = $sql->query("SELECT FGID FROM NC_FleetGroup WHERE FID=$FID");
	return $F[0]['FGID'];
}

function fleet_remove(&$sql, $FID) {
	$FID = makeinteger($FID);
	$FGID = fleet_get_group($sql, $FID);
	$sql->query("DELETE FROM NC_Fleet WHERE FID=$FID");
	fleetgroup_member_drop($sql, $FGID, $FID);
}

function fleet_combine(&$sql, $FID1, $FID2) {
	$fleet1 = fleet_get_all($sql, $FID1);
	$fleet2 = fleet_get_all($sql, $FID2);
	if ($fleet1['Owner'] != $fleet2['Owner'])
		return "Selected fleets belong to different players";
	$G1 = $fleet1['FGID'];
	$G2 = $fleet2['FGID'];
	if ($G1 != $G2) {
		if ($fleet1['Location'] != $fleet2['Location'] or $fleet1['FMID'] != $fleet2['FMID'])
			return "Selected fleets are not in the same place";
		$arr1 = fleetgroup_get_arity($sql, $G1);
		$arr2 = fleetgroup_get_arity($sql, $G2);
		if ($arr1 != 1 or $arr2 != 1)
			return "At least one of the selected fleets is a part of a bigger group";
	}
	$newfleet['Owner']=$fleet1['Owner'];
	global $anyships;
	foreach ($anyships as $S) {
		$newfleet[$S]=$fleet1[$S]+$fleet2[$S];
	}
	fleet_update_arr($sql, $fleet1['FID'], $newfleet);
	fleet_remove($sql, $FID2);
}

function fleet_join(&$sql, $FID1, $FID2) {
	$FGID1 = fleet_get_group($sql, $FID1);
	$FGID2 = fleet_get_group($sql, $FID2);
	fleetgroup_combine($sql, $FGID1, $FGID2);
}
